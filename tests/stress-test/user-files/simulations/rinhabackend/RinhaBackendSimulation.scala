...
class RinhaBackendSimulation
  ...
  setUp(
    criacaoEConsultaPessoas.inject(
      ...
      rampUsersPerSec(12).to(1200).during(3.minutes) // lezzz go!!!
    ),
    buscaPessoas.inject(
      constantUsersPerSec(2).during(25.seconds), // warm up
      
      rampUsersPerSec(12).to(200).during(3.minutes) // lezzz go!!!
    ),
    buscaInvalidaPessoas.inject(
      constantUsersPerSec(2).during(25.seconds), // warm up
      
      rampUsersPerSec(12).to(80).during(3.minutes) // lezzz go!!!
    )
